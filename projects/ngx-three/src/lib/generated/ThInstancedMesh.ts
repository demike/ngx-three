...
import { BufferAttribute, BufferGeometry, InstancedMesh, Material } from 'three';
...
> extends ThMesh<TGeometry, TMaterial, T, TARGS> {
  ...
  public set instanceColor(value: null | BufferAttribute | [value: ArrayLike<number> | ArrayBufferView, offset?: number]) {
    if (this._objRef) {
      this._objRef.instanceColor = applyValue<null | BufferAttribute>(this._objRef.instanceColor, value);
    }
  }
  @Input()
  public set instanceMatrix(value: BufferAttribute | [value: ArrayLike<number> | ArrayBufferView, offset?: number]) {
    if (this._objRef) {
      this._objRef.instanceMatrix = applyValue<BufferAttribute>(this._objRef.instanceMatrix, value);
    }
  }
}
